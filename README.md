...
## 🛠️ 核心技术栈
* [cite_start]**语言**: Go 1.25+ (Alpine) [cite: 9]
* [cite_start]**容器**: Docker / Docker Compose / K3s [cite: 13, 102, 163]
* [cite_start]**监控**: Prometheus / Grafana / Redis-Exporter [cite: 42, 43, 46]
* [cite_start]**交付**: Helm / ArgoCD / Istio [cite: 153, 163, 206]

## ⚠️ 避坑检查点 (Checkpoint)
* [cite_start]**网络**：若出现 i/o timeout，优先检查 `net.ipv4.ip_forward` 是否为 1 [cite: 139-143]。
* [cite_start]**存储**：Redis 多副本挂载 NFS 易冲突，建议使用单副本或 StatefulSet [cite: 131]。
* [cite_start]**镜像**：国内环境拉取失败时，需手动通过 `k3s ctr` 注入 `k8s.io` 命名空间 [cite: 118, 119]。
